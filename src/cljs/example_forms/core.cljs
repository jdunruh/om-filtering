...
;; Code to annotate app-state if it had come from server
(defn annotate-discrete-selection [x]
  (if (= (:type x) :discrete-selection)
    (assoc x :content
             (mapv #(assoc % :func (partial apply-discrete (:field x) (:selector %))) (:content x)))
    x))

(defn annotate-range [x]
  (if (= (:type x) :range)
    (assoc x :content
             (mapv #(assoc % :func (partial apply-range (:field x) (:bottom %) (:top %))) (:content x)))
    x))

(defn annotate-state [state] (assoc state :selectors
                                              (mapv #(-> % annotate-discrete-selection annotate-range) (:selectors state))))